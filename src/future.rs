...
//
// #[derive(Clone, Debug)]
// pub struct FutureSemigroup {}
//
// #[derive(Clone, Debug)]
// pub struct FutureMonad<'a, X=(), Y=(), Z=()> {
//     _p: PhantomData<&'a()>,
//     _a: PhantomData<X>,
//     _b: PhantomData<Y>,
//     _c: PhantomData<Z>
// }
//
// impl<'a, X, Y, Z> FutureMonad<'a, X, Y, Z> {
//     pub fn new(_: Z) -> Self {
//         FutureMonad {
//             _p: PhantomData,
//             _a: PhantomData,
...
// }
//
// impl Effect for FutureSemigroup {}
// impl<'a, X, Y, Z> Effect for FutureMonad<'a, X, Y, Z> {}
//
// impl<'a, X> Monoid<ConcreteFuture<'a, X>> for FutureSemigroup {
//     fn empty() -> ConcreteFuture<'a, X>
//         where
//             MX: 'a + Monoid<X> + Sync + Send {
//         ConcreteFuture::new(ready(MX::empty()))
//     }
// }
//
...
//     ConcreteFuture<'a, X1>,
//     ConcreteFuture<'a, X2>,
//     ConcreteFuture<'a, R>> for FutureSemigroup
//     where
//         X1: 'a + Send + Sync,
//         X2: 'a + Send + Sync,
//         R: 'a + Send + Sync {
//     fn combine<MX>(a: ConcreteFuture<'a, X1>, b: ConcreteFuture<'a, X2>) -> ConcreteFuture<'a, R>
//         where
//             MX: 'a + Semigroup<X1, X2, R> {
//         let fr = a.then(move |i| b.map(move |j| MX::combine(i, j)));
//         ConcreteFuture::new(fr)
//     }
// }
//
// impl<'a, X, Y, Z> Functor<'a> for FutureMonad<'a, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync {
//     type X = X;
...
// }
//
// impl<'a, X, Y, Z> Applicative<'a> for FutureMonad<'a, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync {
...
// }
//
// impl<'a, X, Y, Z, M> ApplicativeApply<'a, M> for FutureMonad<'a, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync,
...
// }
//
// impl<'a, X, Y, Z> Functor2<'a> for FutureMonad<'a, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync {
...
// }
//
// impl<'a, X, Y, Z> Monad<'a> for FutureMonad<'a, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync {
...
// }
//
// impl<'a, X, Y, Z> Foldable<'a> for FutureMonad<'a, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync {
...
// //}
//
// impl<'a, X, Y> Productable<'a> for FutureMonad<'a, X, Y, (X, Y)>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync {}
//
...
//     fn test_semigroup() {
//         block_on(async {
//             let f1: ConcreteFuture<u32> = FutureMonad::pure(1);
//             let f2: ConcreteFuture<u32> = FutureMonad::pure(2);
//             let fr = FutureMonad::combine(f1, f2);
//             assert_eq!(fr.await, 3);
//
//             let f1: ConcreteFuture<u32> = FutureMonad::pure(3);
//             let f2: ConcreteFuture<u32> = FutureMonad::pure(5);
//             let fr = FutureMonad::combine_inner::<IntMulMonoid>(f1, f2);
...
//     fn test_monoid() {
//         block_on(async {
//             let f: ConcreteFuture<u32> = FutureMonad::empty();
//             assert_eq!(f.await, 0);
//         });
//     }
//
...
//     fn test_applicative() {
//         block_on(async {
//             let f: ConcreteFuture<u32> = FutureMonad::pure(3u32);
//             assert_eq!(f.await, 3);
//             let f: ConcreteFuture<Result<&str, ()>> = FutureMonad::pure(Ok("test"));
...
//         block_on(async {
//             let f: ConcreteFuture<u32> = FutureMonad::pure(3u32);
//             let f = FutureMonad::fmap(f, |i| format!("{} strings", i));
...
//         block_on(async {
//             let f: ConcreteFuture<u32> = FutureMonad::pure(3u32);
//             let f2 = FutureMonad::flat_map(f, |i| {
...
//     fn test_product() {
//         block_on(async {
//             let f1: ConcreteFuture<u32> = FutureMonad::pure(3u32);
//             let f2: ConcreteFuture<&str> = FutureMonad::pure("strings");
//             let f = FutureMonad::product(f1, f2);
