...
impl<'a, X, Y, Z> Functor<'a> for OptionEffect<X, Y, Z> {
    ...
    type FnctY = Y;
    type FnctZ = Z;
    type FctForX = Option<X>;
    type FctForY = Option<Y>;
    type FctForZ = Option<Z>;
...
mod tests {
    ...
    #[test]
    fn test_functor() {
        let out: Option<u32> = Some(3);
        let res = fmap(out, |i| i + 4);
        assert_eq!(Some(7), res);

        let out: Option<String> = Some(format!("Hello"));
        let res = fmap(out, |i| format!("{} World", i));
        assert_eq!("Hello World", res.unwrap());

        let out: Option<u32> = None;
        let res = fmap(out, |i| format!("{} World", i));
        assert_eq!(None, res);

        let out1: Option<u32> = Some(3);
        let out2: Option<String> = Some(format!("Bowls"));
