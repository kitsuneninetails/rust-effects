...
//
// #[derive(Clone, Debug)]
// pub struct IoSemigroup {}
//
// pub struct IoMonad<'a, E, X=(), Y=(), Z=()>
//     where
//         X: 'a + Send + Sync,
//         E: 'a + Debug + Send + Sync {
//     _p: PhantomData<&'a()>,
...
// }
//
// impl<'a, E, X, Y, Z> IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         E: 'a + Send + Sync + Debug {
//     pub fn app(_: Z) -> IoMonad<'a, E, X, Y, Z> {
//         IoMonad {
//             _p: PhantomData,
//             _a: PhantomData,
//             _b: PhantomData,
...
//         IO::new(FutureResultSemigroup::combine_futures(a.fut, b.fut, func))
//     }
// }
//
// #[macro_export]
// macro_rules! io_semigroup {
//     () => (IoSemigroup {})
// }
// #[macro_export]
...
// }
//
// impl Effect for IoSemigroup {}
//
// impl<'a, E, X, Y, Z> Effect for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         E: 'a + Debug + Send + Sync {}
//
// impl<'a, E: Debug + Send + Sync, X> Monoid<IO<'a, X, E>> for IoSemigroup
//     where
//         X: 'a + Sync + Send,
//         E: 'a + Sync + Send + Debug {
//     fn empty() -> IO<'a, X, E> {
//         IO::new(FutureResultSemigroup::<E, X>::empty())
//     }
// }
//
// impl<'a, X1, X2, R, E: Debug + Send + Sync> Semigroup<IO<'a, X1, E>, IO<'a, X2, E>, IO<'a, R, E>> for IoSemigroup
//     where
//         X1: 'a + Send + Sync,
//         X2: 'a + Send + Sync,
//         R: 'a + Send + Sync,
//         E: 'a + Send + Sync + Debug {
//     fn combine<MX>(a: IO<'a, X1, E>,
//                b: IO<'a, X2, E>) -> IO<'a, R, E>
//         where
//             MX: Semigroup<IO<'a, X1, E>, IO<'a, X2, E>, IO<'a, R, E>> {
//         Self::combine_futures(a, b, MX::combine)
//     }
// }
//
// impl<'a, E: Debug + Send + Sync, X, Y, Z> Functor<'a> for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         E: 'a + Send + Sync + Debug {
...
//         IO::new(FutureResultMonad::<E, X, Y, Z>::fmap(f.fut, func))
//     }
// }
//
// impl<'a, E: Debug + Send + Sync, X, Y, Z> Applicative<'a> for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync,
//         E: 'a + Send + Sync + Debug {
//     fn pure(x: X) -> Self::FX {
//         IO::new(FutureResultMonad::<E, X, Y, Z>::pure(x))
//     }
// }
//
// impl<'a, E, X, Y, Z, M> ApplicativeApply<'a, M> for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync,
...
//         IO::new(FutureResultMonad::<E, X, Y, Z>::apply(func.fut, x.fut))
//     }
// }
//
// impl<'a, E: Debug + Send + Sync, X, Y, Z> Functor2<'a> for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync,
...
//         IO::new(FutureResultMonad::fmap2(fa.fut, fb.fut, func))
//     }
// }
//
// impl<'a, E: Debug + Send + Sync, X, Y, Z> Monad<'a> for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync,
...
//                     Ok(x_in) => func(x_in),
//                     Err(e) => IoMonad::<E, X, Y, Z>::raise_error(e)
//                 })
//             )
//         )
//     }
// }
//
// impl<'a, E: Debug + Send + Sync, X, Y, Z> Foldable<'a> for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync,
...
//         IO::new(FutureResultMonad::<E, X, Y, Z>::fold(f.fut, init, func))
//     }
// }
//
// impl<'a, E: Debug + Send + Sync, X, Y, Z> MonadError<'a> for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync,
...
//         IO::new(FutureResultMonad::<E, X, Y, Z>::raise_error(err))
//     }
//
...
//             ConcreteFutureResult::new(
//                 f.then(move |x| match x {
//                     Ok(x_in) => IoMonad::<E, X, Y, Z>::pure(x_in),
//                     Err(e) => recovery(e)
//                 })
//             )
//         )
...
// }
//
// impl<'a, E, X, Y> Productable<'a> for IoMonad<'a, E, X, Y, (X, Y)>
//     where
//         E: 'a + Debug + Send + Sync,
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync {}
//
// impl<'a, E, X, Y, Z> SyncT<'a> for IoMonad<'a, E, X, Y, Z>
//     where
//         X: 'a + Send + Sync,
//         Y: 'a + Send + Sync,
//         Z: 'a + Send + Sync,
...
//        IoMonad::<E, X, Y, Z>::flat_map(IO::apply(|| ()), move |_| thunk())
//     }
//
// }
//
...
//     #[test]
//     fn test_apply() {
//         let t: IO<i32, ()> = IoMonad::pure(2);
//         let func: IO<_, ()> = IoMonad::pure(move |x| x + 4);
//         let io: IO<_, ()> = IoMonad::apply(func, t);
//         assert_eq!(Ok(6), IoMonad::attempt(io));
//
//         let t1: IO<i32, ()> = IoMonad::pure(2);
//         let t2: IO<i32, ()> = IoMonad::pure(4);
//         let func: IO<_, ()> = IoMonad::pure(|y| move |x| x + y);
//         let io: IO<_, ()> = IoMonad::apply(func, t1);
//         let io: IO<_, ()> = IoMonad::apply(io, t2);
//         assert_eq!(Ok(6), IoMonad::attempt(io));
//
//         let t1: IO<i32, ()> = IoMonad::raise_error(());
//         let t2: IO<i32, ()> = IoMonad::pure(4);
//         let func: IO<_, ()> = IoMonad::pure(|y| move |x| x + y);
//         let io: IO<_, ()> = IoMonad::apply(func, t1);
//         let io: IO<_, ()> = IoMonad::apply(io, t2);
//         assert_eq!(Err(()), IoMonad::attempt(io));
//     }
//
//     #[test]
//     fn test_sync() {
...
//             4u32
//         };
//         let t: IO<u32, ()> = IoMonad::delay(func);
//         assert_eq!(4, t.run_sync());
//
//         let func = || {
//             println!("Hello");
...
//             IoMonad::pure(4)
//         };
//         let t: IO<i32, ()> = IoMonad::suspend(func);
//         assert_eq!(4, t.run_sync());
//     }
//
//     #[test]
...
//             4
//         });
//         let t: IO<i32, u32> = IoMonad::flat_map(t, |_| IoMonad::raise_error(2u32));
//         let t: IO<i32, u32> = IoMonad::handle_error(t, |_| IoMonad::pure(200));
//         assert_eq!(IoMonad::attempt(t), Ok(200));
//     }
// }
