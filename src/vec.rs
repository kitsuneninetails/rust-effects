...
//         Self::fmap2(fa, fb, |a, b| (a.clone(), b.clone()))
//     }
// }
//
// // impl<'a, TIn, EffIn, TRet, EffRet, AppEffRet, X, Y, T, Z> Traverse<'a, Vec<X>, E, Vec<Y>, FR, X, Y> for VecEffect<X, Y, Z>
// //     where
// //         X: Clone,
// //         Y: Clone,
// //         TIn: F<X>,
...
// //         let init: EffRet = AppEffRet::pure(VecEffect::<Y>::empty());
// //
...
//     #[test]
//     fn test_monoid() {
//         let out: Vec<u32> = VecEffect::empty();
//         assert!(out.is_empty());
//     }
//
//     #[test]
//     fn test_applicative() {
//         let out = VecEffect::pure(3);
//         assert_eq!(vec![3], out);
//         let out: Vec<&str> = VecEffect::pure("test");
//         assert_eq!(vec!["test"], out);
//     }
//
//     #[test]
//     fn test_functor() {
//         let out: Vec<u32> = VecEffect::pure(3);
//         let res = VecEffect::fmap(out, |i| i + 4);
//         assert_eq!(vec![7], res);
//
//         let out: Vec<String> = VecEffect::pure(format!("Hello"));
//         let res = VecEffect::fmap(out, |i| format!("{} World", i));
//         assert_eq!(vec![format!("Hello World")], res);
//
//         let out: Vec<String> = VecEffect::empty();
//         let res = VecEffect::fmap(out, |i| format!("{} World", i));
//         assert!(res.is_empty());
//
//         let out1: Vec<u32> = VecEffect::pure(3);
//         let out2: Vec<String> = VecEffect::pure(format!("Bowls"));
//         let res = VecEffect::fmap2(out1, out2, |i, j| format!("{} {} of salad", i + 4, j));
...
//     fn test_monad() {
//         let out: Vec<u32> = VecEffect::pure(3);
//         let res = VecEffect::flat_map(out, |i| vec![i + 1, i + 2, i + 3]);
...
//         let res: Vec<u32> = VecEffect::flat_map(out, |_i| VecEffect::empty());
//         assert!(res.is_empty());
//
//         let out: Vec<u32> = vec![2, 3, 4];
//         let res = VecEffect::fold(out, 0, |init, i| init + i);
//         assert_eq!(9, res);
//
//         let out: Vec<u32> = VecEffect::empty();
//         let res = VecEffect::fold(out, 0, |init, i| init + i);
...
//         let res = VecEffect::product(out1, out2);
//         assert_eq!(vec![(2, 4), (2, 5), (3, 4), (3, 5)], res);
//
//         let out1: Vec<u32> = vec![2, 3];
//         let out2: Vec<u32> = VecEffect::empty();
//         let res = VecEffect::product(out1, out2);
//         assert!(res.is_empty());
//     }
//
//     #[test]
//     fn test_traverse() {
//         let o = VecEffect::traverse(vec![2, 4, 6], |x| match x % 2 == 0 {
//             true => Some(format!("{}", x)),
//             false => None
//         });
//         assert!(o.is_some());
...
//         let o = VecEffect::traverse(vec![2, 5, 6], |x| match x % 2 == 0 {
//             true => Some(format!("{}", x)),
//             false => None
//         });
//         assert!(o.is_none());
